...
if(isNil {GVAR_UINS "life_sql_id"}) then {
	...
	EXTDB "9:ADD_PROTOCOL:LOG:SPY_LOG:spyglass.log";
	...
	if((EQUAL(EXTDB_SETTINGS("VAC"),1))) then {
		...
		EXTDB "9:START_VAC";
		EXTDB format["9:ADD_PROTOCOL:VAC:%1",FETCH_CONST(VAC_ID)];
...
if(!(EQUAL(life_server_extDB_notLoaded,""))) exitWith {true}; //extDB did not fully initialize so terminate the rest of the initialization process.
...
/* AKI :: Duplikat z pliku misji w initServer.sqf wiersze 11-26
...
// AKI :: Duplikat z pliku misji w initServer.sqf wiersz 28
...
/* AKI :: Airdrop (?) z poprzedniej wersji */
//[] execVM "\life_server\Functions\airdrop\config.sqf";[] execVM "\life_server\Functions\airdrop\fn_generateAirdropAuto.sqf";